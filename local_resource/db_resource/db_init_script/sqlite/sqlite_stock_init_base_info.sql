...
(    
	NO INTEGER NOT NULL,  -- 등록 번호 (자동등록X)  
	DIVISION TEXT DEFAULT "", -- 구분 KOSPI / KOSDAQ
...
*/
CREATE TABLE IF NOT EXISTS STOCK_CSV_CODE_DB
(
  	NO INTEGER NOT NULL,  -- 등록 번호 (자동등록X)  
	STOCK_CODE TEXT NOT NULL, -- 종목코드 
	STOCK_NAME TEXT NOT NULL, -- 종목명
...
(
	...
	STOCK_CODE TEXT NOT NULL, -- 종목코드 
	CURRENT_STOCK_NAME TEXT NOT NULL, -- 현재 종목명
	OLD_STOCK_NAME TEXT NOT NULL, -- 과거 종목명
	REG_DATE INTEGER NOT NULL, -- 등록일자, 변경일자 (INT vs VARCHAR)
	UPDATE_HISTORY TEXT DEFAULT '', -- 업데이트 이력 (최초등록, 종목명변경, 종목삭제, 재등록, 신규 등록)
	MY_MEMO TEXT DEFAULT '', -- 비고 개인 정보 추가 란
